...
class FigureMCPServerSimple {
        ...
        tools: [
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                requirements: {
                  type: 'string',
                  description: '상세 요구사항 또는 배경 설명 (풍부할수록 좋음)'
                },
                qualityLevel: {
                  type: 'string',
                  enum: ['DRAFT', 'STANDARD', 'PREMIUM'],
                  description: '문서 품질 레벨 (DRAFT: 빠른 생성, STANDARD: 균형, PREMIUM: 최고 품질)',
                  default: 'STANDARD'
                },
                includeSourceAnalysis: {
                  type: 'boolean',
                  description: '소스코드 상세 분석 포함 여부 (기본값: true)',
                  default: true
                },
                autoSave: {
                  type: 'boolean',
      ...
      try {
        switch (name) {
          case 'create_document':
            return await this.handleCreateDocumentSingle(args as any);
  ...
  /**
   * 🔄 자동 워크플로우: 3단계를 자동으로 실행 (타임아웃 최적화)
   */
  private async handleCreateDocumentWorkflow(args: {
    ...
    try {
      ...
      // 타임아웃 방지: 최대 섹션 수를 더 제한
      const maxSections = Math.min(args.maxSections || 2, 2); // 기본값을 2로 감소
      const autoSave = args.autoSave !== false;
      
      // 조기 반환 조건 설정 (20초 제한)
      const WORKFLOW_TIMEOUT = 20000;
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('워크플로우 타임아웃 (20초 초과)')), WORKFLOW_TIMEOUT)
      );
      
      // === 1단계: 템플릿 분석 (타임아웃 보호) ===
      console.error(`📋 Step 1/3: 템플릿 분석 중...`);
      const step1Result = await Promise.race([
        this.handleAnalyzeTemplate({
      ...
      // === 2단계: 섹션별 병렬 생성 (타임아웃 최적화) ===
      console.error(`📝 Step 2/3: 섹션 병렬 생성 중... (최대 ${maxSections}개)`);
      ...
      const sectionPromises = sectionsToGenerate.map(async (section, i) => {
        ...
        try {
          const step2Result = await Promise.race([
            this.handleGenerateSection({
              sessionId,
              sectionId: section.id,
              previousSections: {} // 병렬 처리를 위해 의존성 제거
            }),
            new Promise((_, reject) => 
              setTimeout(() => reject(new Error('섹션 생성 타임아웃')), 8000) // 섹션당 8초 제한
            )
          ]) as any;
          ...
          if (!step2Result.isError && step2Result.content?.[0]?.text) {
            ...
            return { sectionId: section.id, content: sectionContent };
          } else {
      ...
      const step3Result = await Promise.race([
        this.handleAssembleDocument({
          sessionId,
          sections: generatedSections,
          saveToFile: autoSave
        }),
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('문서 조합 타임아웃')), 5000) // 조합은 5초 제한
        )
      ]) as any;
...
**🚀 완성**: 실제 소스코드 기반의 전문 개발 산출물이 자동으로 생성되었습니다!`
      ...
      return {
        content: [{
          type: 'text',
          text: `❌ **자동 워크플로우 ${isTimeout ? '타임아웃' : '실패'}**
...
${isTimeout ? `**⏰ 타임아웃 해결방안**:
1. **섹션 수 줄이기**: \`maxSections: 1\`로 재시도
2. **단계별 실행**: 아래 명령어로 단계별 진행
   \`\`\`
   @figure-mcp analyze_template "${args.featureName}" "${args.documentType}"
   @figure-mcp generate_section [sessionId] [sectionId]
   @figure-mcp assemble_document [sessionId]
   \`\`\`
3. **개별 섹션 생성**: 필요한 섹션만 별도 생성

**🚀 빠른 재시도**:
\`\`\`
@figure-mcp create_document_workflow "${args.featureName}" "${args.documentType}" maxSections=1
\`\`\`` : `**확인 사항:**
- 백엔드 서비스가 실행 중인지 확인
- 사이트명이 정확한지 확인
- 네트워크 연결 상태 확인
...
**키워드 생성**:`;
      ...
      // MCP SDK 1.18.2의 올바른 Sampling 방식 사용
      const response = await this.server.createMessage({
        messages: [{
          role: 'user',
          content: {
      ...
      if (response.content.type === 'text') {
        ...
        aiKeywords = keywordText
          ...
          .filter((keyword: string) => keyword.length > 1 && /^[a-z]+$/.test(keyword)) // 영어만
          .slice(0, 15); // 최대 15개
      }

      // AI가 키워드를 제대로 생성하지 못한 경우 기본 키워드 사용
      if (aiKeywords.length === 0) {
        console.error('⚠️ AI 키워드 생성 실패 - 기본 키워드 사용');
        return this.extractBasicKeywords(featureName, requirements, section);
      }
  ...
  /**
   * 기본 키워드 추출 방식 (MCP Sampling 문제 해결 전까지 임시 사용)
   */
  private extractBasicKeywords(
    ...
    const koreanToEnglish: { [key: string]: string[] } = {
      ...
      '관리': ['manage', 'admin', 'control'],
      '검색': ['search', 'find', 'filter'],
      '목록': ['list', 'table', 'grid'],
      '등록': ['register', 'create', 'add'],
      '수정': ['update', 'edit', 'modify'],
      '삭제': ['delete', 'remove', 'drop'],
      '조회': ['view', 'read', 'get'],
      '페이지': ['page', 'view', 'screen'],
      '화면': ['page', 'view', 'screen'],
      '버튼': ['button', 'btn'],
      '폼': ['form', 'input'],
      '테이블': ['table', 'grid'],
      ...
      '댓글': ['comment', 'reply'],
      '파일': ['file', 'upload', 'attachment'],
      '이미지': ['image', 'photo', 'picture'],
      '알림': ['notification', 'alert', 'message'],
      '설정': ['setting', 'config', 'option'],
      '권한': ['permission', 'role', 'auth'],
      '그룹': ['group', 'team'],
      ...
      '보고서': ['report', 'document'],
      '통계': ['statistics', 'stats', 'chart'],
      '데이터': ['data', 'info'],
      '연동': ['integration', 'sync', 'connect'],
      'api': ['api', 'service', 'endpoint'],
      '인터페이스': ['interface', 'api', 'endpoint']
    };
    
    // 한글 키워드를 영어로 변환
    for (const word of featureWords) {
      if (koreanToEnglish[word]) {
        koreanToEnglish[word].forEach(englishWord => keywords.add(englishWord));
    ...
    const sectionKeywords: { [key: string]: string[] } = {
      'impact': ['controller', 'service', 'model', 'api', 'component', 'view'],
      'requirement': ['spec', 'function', 'feature', 'interface'],
      'design': ['class', 'method', 'function', 'component', 'module'],
      'interface': ['api', 'endpoint', 'request', 'response', 'dto']
    };
    ...
    // 기본 기술 키워드 추가
    const techKeywords = ['controller', 'service', 'model', 'component', 'api', 'view', 'page', 'form', 'button'];
    techKeywords.forEach(keyword => keywords.add(keyword));
...
${content}`;
  ...
  /**
   * 🎯 단일 Sampling 기반 문서 생성 (메인 핸들러)
   */
  private async handleCreateDocumentSingle(args: {
    documentType: string;
    featureName: string;
    siteName?: string;
    requirements?: string;
    ...
    try {
      ...
      // 1단계: 풍부한 컨텍스트 수집 (AI 없이, 빠름)
      console.error(`📊 Step 1/3: 풍부한 컨텍스트 수집 중...`);
      const richContext = await this.gatherRichContext(args);
      
      // 2단계: 통합 프롬프트 생성
      console.error(`📝 Step 2/3: 통합 프롬프트 생성 중...`);
      const unifiedPrompt = await this.buildUnifiedPrompt(richContext);
      
      // 3단계: 단일 AI Sampling 실행
      console.error(`🤖 Step 3/3: AI 원샷 생성 중...`);
      const generatedDocument = await this.executeSingleSampling(unifiedPrompt, args.qualityLevel || 'STANDARD');
      
      // 4단계: 결과 처리 및 저장
      const finalDocument = await this.processAndSaveDocument(generatedDocument, richContext, args);
      
      const processingTime = Date.now() - startTime;
      console.error(`✅ 원샷 문서 생성 완료: ${Math.ceil(processingTime / 1000)}초`);
...
**🆔 문서 ID**: \`${documentId}\`
**⏱️ 총 처리 시간**: ${Math.ceil(processingTime / 1000)}초
**📊 품질 레벨**: ${args.qualityLevel || 'STANDARD'}
...
✅ **컨텍스트 수집**: ${richContext.sourceFiles}개 파일 분석
✅ **템플릿 적용**: ${richContext.templateName} 기반
✅ **AI 토큰 사용**: ${finalDocument.tokensUsed}토큰
✅ **문서 길이**: ${finalDocument.contentLength}자

${args.autoSave !== false ? `## 💾 **자동 저장**

**저장 위치**: \`${finalDocument.savedPath}\`
**파일명**: \`${finalDocument.fileName}\`

**🎯 완성**: 실무에서 바로 사용 가능한 전문 개발 산출물이 생성되었습니다!` : '**💡 팁**: `autoSave: true`로 설정하면 자동으로 파일이 저장됩니다.'}`
        }],
        metadata: {
          documentId,
          documentType: args.documentType,
          ...
          processingTime,
          tokensUsed: finalDocument.tokensUsed,
          contentLength: finalDocument.contentLength,
          sourceFilesAnalyzed: richContext.sourceFiles,
          templateUsed: richContext.templateName,
          savedPath: finalDocument.savedPath,
          success: true,
      ...
      const processingTime = Date.now() - startTime;
      console.error(`❌ 원샷 문서 생성 실패 (${processingTime}ms):`, error);
...
**🔧 문제 해결**:
- 백엔드 서비스 상태 확인 (${this.BACKEND_API_URL})
- 워크스페이스 경로 확인
- 네트워크 연결 상태 확인
- 요구사항을 더 구체적으로 작성

**🚀 재시도**: 품질 레벨을 낮춰서 시도해보세요
\`\`\`
...
\`\`\``
  ...
  private async gatherRichContext(args: any) {
    console.error(`📂 컨텍스트 수집 시작...`);
    
    // 템플릿 로드 (기존 API 사용)
    const template = await this.getTemplateFromBackend(args.documentType, args.siteName || this.DEFAULT_SITE_NAME);
    
    // 워크스페이스 정보 수집
    const workspaceInfo = await this.scanWorkspaceStructure();
    
    // 관련 소스코드 분석
    const sourceAnalysis = args.includeSourceAnalysis !== false 
      ? await this.analyzeRelevantSourceCode(args.featureName)
      : null;
    
    // 프로젝트 메타데이터
    const projectMetadata = await this.gatherProjectMetadata();
    ...
    return {
      ...
      projectStructure: workspaceInfo.structure,
      sourceFiles: workspaceInfo.relevantFiles.length,
      
      // 소스코드 분석
      sourceAnalysis,
      relevantFiles: workspaceInfo.relevantFiles,
   ...
   * 📝 통합 프롬프트 생성
   */
  private async buildUnifiedPrompt(context: any): Promise<string> {
...
**품질 레벨**: ${context.qualityLevel || 'STANDARD'}

## 📋 요구사항
${context.requirements || '기본 요구사항에 따라 생성'}
...
${context.sourceAnalysis ? `### 소스코드 분석
${context.sourceAnalysis.summary}

**주요 파일들**:
${context.sourceAnalysis.keyFiles.map((file: any) => 
  `- **${file.path}**: ${file.description}`
).join('\n')}

**관련 함수/클래스**:
${context.sourceAnalysis.relevantComponents.map((comp: any) => 
...
- **언어**: 한국어
- **형식**: 마크다운
- **상세도**: ${qualitySettings.detailLevel}
- **실무 활용도**: 높음 (바로 사용 가능한 수준)

### 필수 포함 사항
1. **구체적인 데이터**: 실제 파일명, 함수명, 클래스명 활용
2. **실무 중심**: 개발팀이 바로 활용할 수 있는 내용
3. **완전성**: 모든 섹션을 빠짐없이 작성
4. **일관성**: 프로젝트 컨텍스트와 일치하는 내용

## 🚀 생성 시작
위의 모든 정보를 바탕으로 **완전한 ${context.documentType} 문서**를 생성해주세요.`;
  }

  /**
   * 🤖 단일 AI Sampling 실행
   */
  private async executeSingleSampling(prompt: string, qualityLevel: string): Promise<any> {
    const tokenSettings = this.getTokenSettings(qualityLevel);
    
    console.error(`🤖 AI 생성 중... (최대 ${tokenSettings.maxTokens} 토큰)`);
    ...
    const response = await this.server.createMessage({
      ...
      maxTokens: tokenSettings.maxTokens
    });
    
    const content = response.content.type === 'text' ? response.content.text : String(response.content);
    
    return {
      content,
      tokensUsed: tokenSettings.maxTokens, // 실제 사용량은 추정
   ...
   * ⚙️ 품질 레벨별 설정
   */
  private getQualitySettings(qualityLevel: string) {
    const settings = {
      DRAFT: {
        instructions: '간결하고 핵심적인 내용 위주로 작성. 기본 구조만 포함.',
        detailLevel: '기본',
        sections: '핵심 섹션만'
      },
      STANDARD: {
        instructions: '균형잡힌 상세도로 실무에서 활용 가능한 수준으로 작성. 모든 섹션 포함.',
        detailLevel: '표준',
        sections: '모든 섹션 완성'
      },
      PREMIUM: {
        instructions: '최고 수준의 상세도와 완성도로 작성. 추가 인사이트와 권장사항 포함.',
        detailLevel: '최고',
        sections: '모든 섹션 + 심화 내용'
      }
    };
    
    return settings[qualityLevel as keyof typeof settings] || settings.STANDARD;
  }

  /**
   * 🎚️ 품질 레벨별 토큰 설정
   */
  private getTokenSettings(qualityLevel: string) {
    const settings = {
      DRAFT: { maxTokens: 1500, estimatedTime: '5-10초' },
      STANDARD: { maxTokens: 2500, estimatedTime: '10-20초' },
      PREMIUM: { maxTokens: 4000, estimatedTime: '20-40초' }
    };
   ...
   * 📊 워크스페이스 구조 스캔
   */
  private async scanWorkspaceStructure(): Promise<any> {
    // 기존의 소스 스캔 로직 재사용
    const resources = await this.scanWorkspaceResources() || [];
    
    return {
      projectName: this.getProjectName(),
      structure: resources.slice(0, 20), // 상위 20개 파일만
      relevantFiles: resources.filter((r: any) => r.uri && (r.uri.includes('.ts') || r.uri.includes('.js') || r.uri.includes('.py')))
    };
  }
   ...
   * 🔍 관련 소스코드 분석
   */
  private async analyzeRelevantSourceCode(featureName: string): Promise<any> {
    const resources = await this.scanWorkspaceResources() || [];
    const relevantFiles = resources.filter((r: any) => 
      r.description?.toLowerCase().includes(featureName.toLowerCase()) ||
      r.uri?.toLowerCase().includes(featureName.toLowerCase())
    ).slice(0, 5); // 최대 5개 파일만
    
    return {
      summary: `${featureName} 관련 ${relevantFiles.length}개 파일 분석`,
      keyFiles: relevantFiles.map((f: any) => ({
        path: f.uri,
        description: f.description || 'No description available'
      })),
      relevantComponents: [] // 향후 확장 가능
    };
  }
   ...
   * 📋 프로젝트 메타데이터 수집
   */
  private async gatherProjectMetadata(): Promise<any> {
    return {
      timestamp: new Date().toISOString(),
      workspacePath: this.currentWorkspacePath || 'Unknown',
      resourceCount: this.currentWorkspaceResources?.length || 0
    };
  }
   ...
   * 💾 문서 처리 및 저장
   */
  private async processAndSaveDocument(generatedDoc: any, context: any, args: any): Promise<any> {
    ...
    if (args.autoSave !== false) {
      savedPath = await this.saveDocumentToFile(generatedDoc.content, fileName, context);
    }
    
    return {
      content: generatedDoc.content,
      tokensUsed: generatedDoc.tokensUsed,
      contentLength: generatedDoc.content.length,
      fileName,
      savedPath,
